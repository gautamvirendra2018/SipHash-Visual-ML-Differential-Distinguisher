...
def prepare_data(data_loader):
    ...
    for images, labels in data_loader:
        # Flatten raw images into vectors
...
# -------------------
# Training function (Random Forest)
# -------------------
def train_one_round(train_dir, test_dir):
    # Only raw tensors, no resize/normalize
    ...
    model = RandomForestClassifier(
        ...
        max_depth=100,        # let trees expand fully
        random_state=42,
...
BASE_DIR = "siphash_datasets_1blocks_tag_rp" # folder containing RP images

os.makedirs("result", exist_ok=True)
result_file = os.path.join("result", "result_rf.txt")
...
with open(result_file, "w", encoding="utf-8") as f:
    if not os.path.exists("results_mlp.txt") or os.path.getsize("results_mlp.txt") == 0:
