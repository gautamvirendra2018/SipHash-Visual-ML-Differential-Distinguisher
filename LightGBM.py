...
# -------------------
# Helper flatten images for sklearnlightgbm
# -------------------
def prepare_data(data_loader):
    X, y = [], []
    for images, labels in data_loader:
        images = images.view(images.size(0), -1)  # flatten to 1D
        X.extend(images.numpy())
        y.extend(labels.numpy())
    return X, y

# -------------------
# Training function (LightGBM)
# -------------------
def train_one_round(train_dir, test_dir):
    # Just convert to tensors (no resizecrop here)
...
result_file = os.path.join("result", "result_lgbm.txt")
